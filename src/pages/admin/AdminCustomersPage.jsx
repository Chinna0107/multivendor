  ...
  return (
    <div className="w-full max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          <h1 className="font-serif text-2xl sm:text-3xl font-bold text-[#036e26]">Customers</h1>
          <p className="text-[#036e26]/40 text-xs font-sans mt-0.5">{customers.length} total users</p>
        </div>
      </div>

      <div className="relative mb-6">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#036e26]/40" />
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search customers by name, email, or phone..."
          className="w-full pl-9 pr-4 py-3 rounded-xl bg-white border border-[#036e26]/10 text-[#036e26] font-sans text-sm focus:outline-none focus:border-[#036e26]/30 shadow-sm" />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#036e26]/10 overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-sm font-sans min-w-[600px]">
            <thead>
              <tr className="bg-[#FDFBF7] text-[#036e26]/60 text-xs uppercase tracking-wider border-b border-[#036e26]/10">
            ...
            <tbody className="divide-y divide-[#036e26]/5">
                ...
                <motion.tr key={customer.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }}
                  ...
                  <td className="py-4 px-4 sm:px-6">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-[#036e26]/10 flex items-center justify-center text-[#036e26] font-bold">
                        {(customer.name || "U")[0].toUpperCase()}
                      </div>
                      <div>
                        <p className="font-semibold text-[#036e26]">{customer.name || "Unknown User"}</p>
                  ...
                  <td className="py-4 px-4 sm:px-6 space-y-1">
                    <div className="flex items-center gap-1.5 text-xs text-[#036e26]/70">
                  ...
                  <td className="py-4 px-4 sm:px-6 text-xs text-[#036e26]/60">
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={4} className="py-12 text-center text-[#036e26]/50">
                    No customers found
                  </td>
                </tr>
              )}
