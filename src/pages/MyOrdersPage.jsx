    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-2xl font-serif font-bold text-brand-orange">Order History</h2>
          <span className="text-sm font-semibold text-brand-orange/60 bg-brand-green/10 px-3 py-1 rounded-full">{orders.length} Orders</span>
        </div>
        ...
        {orders.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4 bg-white rounded-3xl border border-gray-100 shadow-sm">
            ...
            <p className="text-brand-orange font-bold text-lg">No orders yet</p>
            <p className="text-sm text-brand-orange/50 text-center max-w-sm">Looks like you haven't made your first order. Explore our spiritual collection today!</p>
          ...
          orders.map((order) => {
            ...
            return (
              <div key={order.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
                {/* Order Header */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 py-4 border-b border-gray-100 bg-brand-green">
                  <div>
                    <p className="text-sm font-bold text-brand-orange">Order #{order.order_number || order.id}</p>
                    <p className="text-xs text-brand-orange/60 mt-1">
                  ...
                  <div className="px-6 py-6 border-b border-gray-100 bg-white">
                    <div className="max-w-2xl mx-auto">
                        ...
                        {STATUS_STEPS.map((step, i) => (
                          <div key={step} className="flex flex-col items-center flex-1 relative">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all z-10 ${
                              i <= stepIdx ? 'bg-brand-green border-brand-orange text-white shadow-md' : 'bg-white border-gray-200 text-gray-400'
                            }`}>
                              {i < stepIdx ? '✓' : i + 1}
                            </div>
                            {i < STATUS_STEPS.length - 1 && (
                              <div className={`absolute top-4 left-1/2 w-full h-0.5 -z-0 ${i < stepIdx ? 'bg-brand-green' : 'bg-gray-200'}`} />
                            )}
                          </div>
                        ))}
                      </div>
                      <div className="flex items-center mt-2">
                        {STATUS_STEPS.map((step, i) => (
                          <span key={step} className={`text-[10px] sm:text-xs font-bold capitalize flex-1 text-center ${i <= stepIdx ? 'text-brand-orange' : 'text-gray-400'}`}>
                            {step}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Array.isArray(order.items) && order.items.map((item, i) => (
                      <div key={i} className="flex items-start gap-4 p-3 rounded-xl border border-gray-50 hover:bg-gray-50 transition-colors">
                        <div className="w-16 h-16 bg-brand-green rounded-xl flex items-center justify-center shrink-0 border border-brand-orange/10 overflow-hidden">
                          {item.image_url || item.product?.image_url ? (
                            ...
                            <Package className="w-8 h-8 text-brand-orange/40" />
                          )}
                        </div>
                        <div className="flex-1 min-w-0 pt-1">
                          <p className="text-sm font-bold text-brand-orange line-clamp-1">{item.name || item.product?.name || 'Product'}</p>
                          <div className="flex items-center gap-3 mt-1.5">
                            <span className="text-xs text-brand-orange/60 bg-white px-2 py-0.5 rounded-md border border-gray-100">Qty: {item.qty || 1}</span>
                            {(item.size || item.variant?.size) && (
                              <span className="text-xs text-brand-orange/60 bg-white px-2 py-0.5 rounded-md border border-gray-100">{item.size || item.variant?.size}</span>
                            )}
                          </div>
                        </div>
                        <p className="text-sm font-bold text-brand-orange pt-1">₹{Number(item.price || item.variant?.price || item.product?.price || 0).toLocaleString('en-IN')}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Footer Actions */}
                <div className="flex flex-col sm:flex-row items-center justify-end gap-3 px-6 py-4 bg-brand-green border-t border-brand-orange/10">
                  <button onClick={() => openInvoice(order)}
                    className="w-full sm:w-auto flex items-center justify-center gap-2 bg-white text-brand-orange border border-brand-orange/20 px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-brand-green/5 transition-colors shadow-sm">
                    <FileText className="w-4 h-4 text-brand-orange" /> Download Invoice
                  </button>
                  <button onClick={() => handleReorder(order)}
                    className="w-full sm:w-auto flex items-center justify-center gap-2 bg-brand-green text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-[#004012] transition-colors shadow-sm">
