...
export function HomePage() {
  ...
  return (
    <div ref={container} className="bg-[#f1f3f6] min-h-screen pb-12">
      <Header variant="home" />
      
      {/* 1. Categories Ribbon */}
      <div className="bg-white shadow-sm mb-4 px-2 py-3 overflow-x-auto hide-scrollbar">
        <div className="flex gap-4 md:gap-10 justify-start md:justify-center min-w-max mx-auto px-4">
          {categories.map(cat => (
            <Link key={cat.id} to={`/category/${cat.id}`} className="flex flex-col items-center gap-2 group">
              <div className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-gray-50 flex items-center justify-center border border-gray-100 overflow-hidden group-hover:shadow-md transition-shadow">
                {cat.image_url ? (
              ...
              <span className="text-[11px] md:text-[12px] font-semibold text-gray-800 text-center group-hover:text-[#fe6603]">{cat.name}</span>
            </Link>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-[1400px] mx-auto w-full">
        ...
        <div className="animate-section mx-2 md:mx-4 mb-8 flex flex-col gap-8 md:gap-10">
          ...
          {products.filter(p => p.is_offer).length > 0 && (
            <div>
              <div className="flex justify-between items-center px-2 md:px-0 py-2 mb-2">
                <h3 className="text-lg md:text-xl font-bold text-gray-900">Top Picks For You</h3>
                <Link to="/collection/top-picks" className="bg-[#fe6603] text-white text-[10px] md:text-xs font-bold px-4 py-2 rounded shadow-sm hover:bg-orange-600 transition-colors">VIEW ALL</Link>
              </div>
          ...
          {products.filter(p => p.is_bestseller).length > 0 && (
            <div>
              <div className="flex justify-between items-center px-2 md:px-0 py-2 mb-2">
                <h3 className="text-lg md:text-xl font-bold text-gray-900">Best Sellers</h3>
                <Link to="/collection/best-sellers" className="bg-[#fe6603] text-white text-[10px] md:text-xs font-bold px-4 py-2 rounded shadow-sm hover:bg-orange-600 transition-colors">VIEW ALL</Link>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 md:gap-4">
          ...
          {/* Promotional Interstitial Banner */}
          <div>
            <div className="relative w-full h-[140px] md:h-[220px] rounded-lg md:rounded-2xl overflow-hidden shadow-lg border border-gray-100 group cursor-pointer">
              <img src={imgAarti} alt="Special" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
              <div className="absolute inset-0 bg-gradient-to-r from-brand-orange/95 via-brand-orange/80 to-brand-green/40 flex flex-col justify-center px-8 md:px-16">
                <h2 className="text-white text-xl md:text-4xl font-extrabold drop-shadow-lg mb-3 md:mb-4 max-w-lg leading-snug">
                  Top Offers on Electronics & Fashion
                </h2>
                <Link to="/collection/top-picks" className="bg-white text-brand-orange text-[10px] md:text-sm font-bold px-6 md:px-8 py-2 md:py-3 rounded-full shadow-md w-fit hover:scale-105 transition-transform uppercase tracking-wider">
                  Explore Now
                </Link>
              </div>
            </div>
          ...
          {products.filter(p => p.is_trending).length > 0 && (
            <div>
              <div className="flex justify-between items-center px-2 md:px-0 py-2 mb-2">
                <h3 className="text-lg md:text-xl font-bold text-gray-900">Trending</h3>
                <Link to="/collection/trending" className="bg-[#fe6603] text-white text-[10px] md:text-xs font-bold px-4 py-2 rounded shadow-sm hover:bg-orange-600 transition-colors">VIEW ALL</Link>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 md:gap-4">
          ...
          {categories.map((cat) => {
            ...
            return (
              <div key={cat.id}>
                <div className="flex justify-between items-center px-2 md:px-0 py-2 mb-2">
                  <h3 className="text-lg md:text-xl font-bold text-gray-900">{cat.name}</h3>
                  <Link to={`/category/${cat.id}`} className="text-brand-orange text-[10px] md:text-xs font-bold hover:underline transition-all">VIEW ALL {cat.name.toUpperCase()}</Link>
                </div>
